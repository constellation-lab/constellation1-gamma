...
impl Contract {
    fn initialize(&self, deps: DepsMut, env: Env, info: MessageInfo, msg: InstantiateMsg) -> Result<(), ContractError> {
        ...
        cw20_base::contract::instantiate(
            ...
            info,
            InstantiateMsg {
                name: "Constella Pool Share Token".to_string(),
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Burn { amount } => {
            cw20_execute(deps, env, info, ExecuteMsg::Burn { amount })
                .map_err(|err| ContractError::Std(StdError::generic_err(format!("{}", err))))
        }
        ExecuteMsg::Send { contract, amount, msg } => {
            cw20_execute(deps, env, info, ExecuteMsg::Send { contract, amount, msg })
                .map_err(|err| ContractError::Std(StdError::generic_err(format!("{}", err))))
        }
        ...
        ExecuteMsg::IncreaseAllowance { spender, amount, expires } => {
            cw20_execute(deps, env, info, ExecuteMsg::IncreaseAllowance { spender, amount, expires })
                .map_err(|err| ContractError::Std(StdError::generic_err(format!("{}", err))))
        }
        ExecuteMsg::DecreaseAllowance { spender, amount, expires } => {
            cw20_execute(deps, env, info, ExecuteMsg::DecreaseAllowance { spender, amount, expires })
                .map_err(|err| ContractError::Std(StdError::generic_err(format!("{}", err))))
        }
        ...
        ExecuteMsg::BurnFrom { owner, amount } => {
            cw20_execute(deps, env, info, ExecuteMsg::BurnFrom { owner, amount })
                .map_err(|err| ContractError::Std(StdError::generic_err(format!("{}", err))))
        }
        ExecuteMsg::SendFrom { owner, contract, amount, msg } => {
            cw20_execute(deps, env, info, ExecuteMsg::SendFrom { owner, contract, amount, msg })
                .map_err(|err| ContractError::Std(StdError::generic_err(format!("{}", err))))
        }
        _ => execute_stake(deps, env, info, Uint128::zero()), // Handle custom messages here
    }
}
...
const INITIAL_SUPPLY: u128 = 1_000_000_000; // Set the initial supply of CPST tokens
