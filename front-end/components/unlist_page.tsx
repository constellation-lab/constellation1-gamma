    ...
    return (
        ...
        <>
          ...
          <PopoverContent color='white' bg='blue.800' borderColor='blue.800' >
            ...
            <PopoverBody>
            <VStack align="start" fontWeight="bold" fontSize={{ md: 'lg' }}  mb={1}>
                <Flex justify="space-between" w = "full"><Text>List Price is:</Text><Text>{Number(data.price)/1000000}NIBI</Text></Flex> 
           </VStack>        
...
const UpdatePriceButton = ({
    ...
    return (
        ...
        <>
          <PopoverTrigger>
            <Button isLoading={isSubmitting} colorScheme="primary" isDisabled={(Date.now()>expires/1000000)} >Udate Pice</Button>
          </PopoverTrigger>
          <PopoverContent color='white' bg='blue.800' borderColor='blue.800' >
            {data.price[0]?( <PopoverHeader pt={4} fontWeight='bold' border='10' >
              current price is: {Number(data.price)/1000000} NIBI
...
export const ManageOptionList = ()=>{
    ...
    <Box>
        <VStack spacing={5}>
        <Button onClick={handleQueryOwnerList} w="full" justifyContent="center" >Refresh the options you LIST</Button>
        {datas?(
            datas.map((data)=>{
                return(
                        <OptionCard data={data[1]} id={data[0]} key = {data[0]}/>
