...
import {ExecuteMsg as MarketMsg,ContellaMarketClient} from "../config/market"
...
export const useMarketTx = (chainName: string,denom: string,contractAddress:string) => {
  const { address, getSigningCosmWasmClient } =
    useChain(chainName);
  ...
  const tx = async (msgs: MarketMsg,options: TxOptions, _funds?: Coin[]) => {
    if (!address) {
      toaster.toast({
        type: ToastType.Error,
        title: 'Wallet not connected',
    ...
    let client: Awaited<ReturnType<typeof getSigningCosmWasmClient>>;
    let wasmClient: Awaited<ContellaMarketClient>;
    let fee: StdFee;
    try {
      fee = {
        amount:[{denom:denom,amount:"125000"}],
      ...
      client = await getSigningCosmWasmClient();
      wasmClient = new ContellaMarketClient(client,address,contractAddress)
    } catch (e: any) {
      console.error(e);
      toaster.toast({
        title: TxStatus.Failed,
